    ...
    <div class="menuText form">
        <form method="POST" action="{{url('/admin/service/edit/updateService')}}" enctype="multipart/form-data">
            ...
            <li>
                    ...
                    <button class="buttonImageAdmin button inputForm" type="button" onclick="document.getElementById('profilePhotoEdit').click()" class="dugmeFile">Ažuriraj sliku</button>
                ...
                <li><label>Naziv</label><input type="text" name="name"  id="serviceName"  value="{{$service->name}}" /></li>
                <li><label>Link</label><input type="text" name="link"  id="serviceLink"  value="{{$service->link}}" /></li>
                <li><label>Tehnologije</label><textarea id="serviceText" name="text">{{$service->text}}</textarea></li>
                <li><label>Nivo </label><select name="level">
                    @foreach ($level as $item)
                        @component("admin.partials.levelEdit",["item"=>$item,"level"=>$service->idLevel])

                        @endcomponent
                    @endforeach
                    </select></li>
                <li><button name="editService" id="submit" class="buttonImageAdmin button inputForm"type="submit">Potvrdi</button></li>
